...
$query132 = mysqli_query($conn, "SELECT * FROM examdeets WHERE emal= '$plicantmail'");
$check = mysqli_num_rows($query132);
if ($check!= 1){ 
// do not insert
echo $mindcheck23 = "<script type ='text/javascript'>
                      alert('you have not yet filled your olevel records, please do before proceeding to Jamb result upload .');
                      window.location.href='uploadolevel1.php';
                         </script>";}


$query134= mysqli_query($conn, "SELECT * FROM jambrecord 
  WHERE studentid ='$plicantid'");
$countrow = mysqli_num_rows($query134);
  if ($countrow > 0){ 
// do not insert
header("Location:viewjamb.php");
}
...
$counter = 1;                     
 while ($counter < 5) {
  ...
  if($subval !=""){
    $newme = mysqli_query($conn, "INSERT INTO jambrecord (Jambreg,studentid,subject,mark) VALUES ('$jambreg','$plicantid','$subval','$graval')");
  }    
    $counter++;          
}   
 ...
 //VALUES('$plicantid','$subject1','$grading1')");
  ...
  $reportalert="<script type ='text/javascript'>
                      alert('Jamb results  successfully uploaded');
  ...
  ?>
                              ...
                              <option value="<?php echo $cussin;?>"><?php echo $cussin;?></option> 
                                ...
                                <div class="form-group col-lg-4">
                                    <label>Score</label>
                                <input type="text" name="mark<?php echo $numberz;?>">
